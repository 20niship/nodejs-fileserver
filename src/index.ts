...
if (needLogin || needLoginToEdit) {
  ...
  app.get("/logout", (req, res) => {
    req.session.destroy((_: any) => { res.redirect('/'); });
  })
}
...
// file cache
if (parseInt(process?.env?.expireTime || "-1") > 0) {
  app.use((_, res, next) => {
    const t = process?.env?.expireTime || "-1";
    // res.header('Expires', new Date(Date.now() + process.env.expireTime).toUTCString());
    res.header("Cache-Control", "public, max-age=" + t.toString());
    next()
  })
}
...
const formatDate = (dt: Date) => {
  try {
    ...
    console.error("Unknown datetime --> 0000-00-00")
    return "0000-00-00"
  }
}

const getMineType = (fname: string): string => {
  const sp = fname.split(".").pop();
  if (sp === undefined) return "";
  const extention = sp.toLowerCase();
...
const getFileType = (fname: string) => {
  const sp = fname.split(".").pop();
  if (sp === undefined) return "";
  const extention = sp.toLowerCase();
  ...
  for (let i in filetypes) if (filetypes[i].indexOf(extention) >= 0) { return i }
  return "";
}

const ftype2icon = (ftype: string) => {
  const f2i: { [key: string]: string } = {
...
const filesize_format = (size: number) => {
  var i = Math.floor(Math.log(size) / Math.log(1024));
  return (size / Math.pow(1024, i)).toFixed(2) + ' ' + ['B', 'kB', 'MB', 'GB', 'TB'][i];
}

const view = async (req: express.Request, res: express.Response, is_trash: boolean) => {
  const dir = (req?.query?.dir as string) || "/";
  const location = decodeURIComponent(dir).replace(/[\/]{2,}/gm, "/").replace(/(.^\/+)[\/]{1,}$/gm, "(.+)")
  ...
  const render_title_html = (_title: string) => {
  ...
  if (stat === false) {
    ...
    for (let i in names) {
      ...
      if (filetype === "") {
        const stat = await fs.stat(`${fname}/${names[i]}`).catch((_: any) => { return false; });
...
app.use('/src', express.static(process?.env?.DIR || "/src"));
app.use('/src-trash', express.static(process?.env?.TRASH_DIR || "/trash"));
app.use('/static', express.static("static"));
app.use("/api", require("./api.ts"))
app.use((_, res, __) => { res.status(404).send("<h1>Page Not Found!</h1>") });
...
if (process.env.BACKUP === "true") {
  ...
  schedule.scheduleJob('0 1 * * *', (fireDate: string) => {
    INFO(`Backup doing ..... --> scheduled = ${fireDate}, destination = ${process.env.BACKUP_DIR}`)
    fs_extra.copy(process.env.DIR, process.env.BACKUP_DIR, (err: any) => {
